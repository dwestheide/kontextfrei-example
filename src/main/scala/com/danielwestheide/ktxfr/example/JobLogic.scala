...
class JobLogic[DCollection[_]: DCollectionOps] {
  ...
  def languagesByPopularity(
      starred: DCollection[RepoStarred],
      created: DCollection[RepoCreated]): DCollection[(String, Long)] = {
    val starsByIdentity = starred
      .map(evt => (evt.owner, evt.repo) -> 1L)
    val languagesByRepoIdentity =
      created.map(evt => (evt.owner, evt.name) -> evt.mainLanguage)
  ...
  def parseRepoStarredEvents(
      csv: DCollection[String]): DCollection[RepoStarred] = {
    csv.map { evt =>
  ...
  def parseRepoCreatedEvents(
      csv: DCollection[String]): DCollection[RepoCreated] = {
    csv.map { evt =>
      evt.split(",").toList match {
