...
trait LanguagesByPopularityProperties[DColl[_]] extends BaseSpec[DColl] {
  ...
  property("Each language from created events has a star count") {
    forAll(createdAndStarredEvents) {
      case (createdEvts, starredEvts) =>
        whenever(starredEvts forall existingRepo(createdEvts)) {
          val result = logic
            .languagesByPopularity(unit(starredEvts), unit(createdEvts))
            .collect()
            .toList
          result.map(_._1).toSet mustEqual createdEvts
            .map(_.mainLanguage)
            .toSet
        }
    }
  ...
  private def existingRepo(createdEvts: List[RepoCreated])(
      evt: RepoStarred): Boolean = {
